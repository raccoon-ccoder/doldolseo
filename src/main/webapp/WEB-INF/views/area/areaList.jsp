...
<body>
    ...
    <section id="areaList-container">
        <!--지역별 배경사진-->
        <div id="areaList-img">
            <img src="_image/area/areaImage_${sigungu}.png" width="1500" height="550px">
        </div>
            ...
            <div id="areaList-dataPage">
                <table class="pagination">
                    <tr>
                        <!-- 첫 페이지로 이동 -->
                        <td>
                            <a href="/areaL?sigungu=${sigungu}&page=0"> << </a>
                        </td>

                        <!-- 이전 페이지로 이동 : 첫 페이지 제외 -->
                        <c:if test="${startBlockPage ne 1}">
                            <td>
                                <a href="/areaL?sigungu=${sigungu}&page=${startBlockPage-2}"> < </a>
                            </td>
                        </c:if>
                        ...
                        <c:forEach begin="${startBlockPage}" end="${endBlockPage}" var="idx">
                            <td>
                                <a href="/areaL?sigungu=${sigungu}&page=${idx-1}">${idx}</a><!-- 이거 현재 URL 로 반영되게 바꿀것 -->
                            </td>
                        </c:forEach>
                        ...
                        <c:if test="${endBlockPage ne areaList.totalPages}">
                            <td>
                                <a href="/areaL?sigungu=${sigungu}&page=${endBlockPage}"> > </a>
                            </td>
                        </c:if>

                        <!-- 마지막 페이지로 이동 -->
                        <td>
                            <a href="/areaL?sigungu=${sigungu}&page=${areaList.totalPages-1}"> >> </a>
                        </td>
                    </tr>
                </table>
            </div>
